...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // expose initial values to global scope so other script blocks can access them
  window.selectedUnit = selectedUnit;
  window.lastFetchedUnitPrice = lastFetchedUnitPrice;
  window.isFetchingPrice = isFetchingPrice;
  ...
  unitOptions.forEach(option => {
    option.addEventListener('click', function() {
      ...
      // keep global in sync for other blocks
      window.selectedUnit = selectedUnit;
      fetchAndUpdatePrice();
      updateSummary();
    });
  ...
  // reflect to global immediately so other blocks see the flag
  window.isFetchingPrice = isFetchingPrice;
    return fetch(url, {
      ...
      .then(data => {
        ...
        // update global copy so other blocks can read latest price
        window.lastFetchedUnitPrice = lastFetchedUnitPrice;
      ...
      .catch(err => {
        ...
        lastFetchedUnitPrice = 0;
        // update global price too
        window.lastFetchedUnitPrice = lastFetchedUnitPrice;
        updatePriceDisplay();
        validateForm();
      })
      .finally(() => { isFetchingPrice = false; window.isFetchingPrice = false; });
  }
  ...
  document.addEventListener('change', function(e) {
    ...
    if (t.matches && (t.matches('input[type="radio"][name^="time-"]') || t.matches('input[name="timeSlot"]'))) {
      try {
        ...
        // Use globals exported from the main block so this handler doesn't throw
        const globalUnit = window.selectedUnit;
        const globalPrice = Number(window.lastFetchedUnitPrice || 0);
        const globalFetching = !!window.isFetchingPrice;
        // If we have a selected unit and no price loaded, fetch price first
        if (globalUnit && cityVal && (globalPrice <= 0) && !globalFetching) {
          // fetch will call updatePriceDisplay when done
          if (typeof window.fetchAndUpdatePrice === 'function') window.fetchAndUpdatePrice();
        } else {
          if (typeof window.updatePriceDisplay === 'function') window.updatePriceDisplay();
        }
      } catch (err) {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    fetch(`/get-city-slots/${city}`)
      ...
      .then(data => {
        ...
        if (timeSlotContainer) {
          // Clear existing children safely (avoid direct innerHTML='' to preserve JS stability)
          ...
          console.warn('No #timeSlotContainer element found — skipping global time-slot render.');
        }
        // expose useful functions to global scope for other inline script blocks
        try {
          window.updatePriceDisplay = updatePriceDisplay;
          window.fetchAndUpdatePrice = fetchAndUpdatePrice;
          window.updateAllSelectOptions = updateAllSelectOptions;
          window.redistribute = redistribute;
        } catch (e) {
          // ignore if any of these are undefined for some reason
        }
      });
  });
  ...
  daysCheckboxes.forEach(cb => cb.addEventListener('change', function() {
      if (typeof window.updatePriceDisplay === 'function') window.updatePriceDisplay(); else calculateTotal();
    }));
  monthsSelect.addEventListener('change', function() {
      if (typeof window.updatePriceDisplay === 'function') window.updatePriceDisplay(); else calculateTotal();
  });
